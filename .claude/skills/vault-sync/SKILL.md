...
- `distractor_count`/`correct_count` mỗi **DIS note, topic note, trap note VÀ sequence note** từ history (cùng cơ chế — đếm role trong `history`).
...
Trong mỗi grammar/pair/distractor/topic/trap/sequence note, tìm block:
