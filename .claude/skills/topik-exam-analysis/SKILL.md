...
description: Phân tích đề TOPIK (ảnh hoặc text) thành EXAM note + diff cho các layer Grammar/Pair/Group/Distractor/Topic/Trap/Sequence/Blank của learning-vault (domain: topik). Xử lý câu ngữ pháp (ngữ-pháp-chỗ-trống, ngữ-pháp-đồng-nghĩa), câu đọc-hiểu chủ đề (xác-định-chủ-đề — "무엇에 대한 글인지", ĐỌC câu 5~8), câu đọc-hiểu "내용과 같은 것" (câu-giống-đoạn-văn câu 9 poster + câu-giống-đồ-thị câu 10 đồ thị — sinh song song trap-note + topic-note), và câu sắp xếp thứ tự (sắp-xếp-thứ-tự — "순서대로 맞게 배열한 것", ĐỌC câu 13~15, 4 mảnh 가~라 — sinh sequence-note), và câu điền nội dung chỗ trống (điền-nội-dung — "빈칸에 들어갈 내용으로 가장 알맞은 것", ĐỌC câu 16~18, 4 mệnh đề nội dung — sinh blank-note), và câu điền phó từ chỗ trống (điền-phó-từ — "( )에 들어갈 알맞은 것", ĐỌC câu 19, 4 phó từ liên kết 또한/오히려/게다가/만약 — sinh adverb-note; câu 20 cùng đoạn văn = câu-giống-đoạn-văn-dài). Dùng skill này BẤT CỨ KHI NÀO người dùng đưa đề TOPIK mới, ảnh câu hỏi trắc nghiệm tiếng Hàn (ngữ pháp, quảng cáo/thông báo, poster, đồ thị, sắp xếp câu, hoặc điền phó từ/nội dung), yêu cầu "phân tích đề", "backfill đề cũ", hoặc nhắc đến việc thêm câu hỏi vào vault — kể cả khi họ chỉ dán một câu hỏi lẻ. Không dùng cho câu hỏi lý thuyết ngữ pháp thuần túy không gắn với đề.
---
...
- **Dạng bài** (slug tra ở `TOPIK/90_MOC/MOC_Dang_Bai.md` — không lưu trong frontmatter, chỉ ghi ở heading `## Câu {N} — dạng {slug} ({hậu tố 한국어})`; hậu tố tra cột "Hậu tố heading" trong `MOC_Dang_Bai`):
  ...
  - `điền-nội-dung` [ĐỌC câu 16~18]: "다음을 읽고 (   )에 들어갈 내용으로 가장 알맞은 것을 고르십시오" — đoạn văn có **chỗ trống ( )**; 4 phương án là **mệnh đề/cụm nội dung** (KHÔNG phải ngữ pháp/danh từ chủ đề/câu khẳng định/hoán vị). Trích thêm: **toàn văn đoạn văn** (giữ `( )` đúng vị trí) + dịch, câu ngay trước/sau chỗ trống, nghĩa từng phương án, đáp án từ MOC. **Xác định quan hệ suy luận** của đáp án đúng (blank khớp: căn cứ/lý do đã nêu, kết quả nhân-quả 따라서/그래서, tương phản 그러나/하지만, khái quát 즉/이처럼, mục đích -기 위해, điều kiện -면, ví dụ, so sánh) → map category enum (`căn-cứ|nhân-quả|đối-lập|khái-quát|mục-đích|điều-kiện|ví-dụ|so-sánh`). Ghi dòng `**Quan hệ:** {quan hệ} → [[BLK_{category}]]` mức câu (xem 2f).
  - `điền-phó-từ` [ĐỌC câu 19]: "( )에 들어갈 알맞은 것을 고르십시오" — đoạn văn có **chỗ trống ( )** **dùng CHUNG với câu 20**; 4 phương án là **phó từ liên kết/phó từ câu** (또한/오히려/아마/특히/물론/게다가/만약… — KHÔNG phải ngữ pháp/nội dung/danh từ chủ đề/hoán vị). Trích thêm: **toàn văn đoạn văn** (giữ `( )` đúng vị trí) + dịch, câu ngay trước/sau chỗ trống, nghĩa 4 phó từ, đáp án từ MOC. **Xác định chức năng phó từ đáp án đúng** (bổ-sung 또한/게다가; đối-lập 오히려/반면에; nhân-quả 그래서/따라서; điều-kiện 만약…-면/다면; khái-quát 즉/이처럼; suy-đoán 아마; nhấn-mạnh 특히/과연; nhượng-bộ 물론…그런데) → map category enum (`bổ-sung|đối-lập|nhân-quả|điều-kiện|khái-quát|suy-đoán|nhấn-mạnh|nhượng-bộ|lựa-chọn|trình-tự-thời-gian`). Ghi dòng `**Chức năng:** {chức năng} → [[ADV_{category}]]` mức câu (xem 2g). **Câu 20 cùng đoạn văn** = dạng `câu-giống-đoạn-văn-dài` → chú giải chi tiết đặt ở câu 19, câu 20 trỏ về.
...
### 2f. Câu dạng `điền-nội-dung` (câu 16~18 — single-layer blank-note, Q20)

Dạng điền nội dung chỗ trống sinh **một layer duy nhất `blank-note`** (`TOPIK/48_Blanks/`, tổ chức **theo quan hệ suy luận chỗ trống**, 1 file = 1 quan hệ; taxonomy trong `frontmatter_schema.md` §blank-note). KHÔNG đụng GR/PAIR/GROUP/DIS/TOPIC/TRAP/SEQ (VC vẫn tạo ở 2d khi có cụm lexical-hóa; link `[[GR_*]]` nền khi note tồn tại).

- Với mỗi câu, xác định **quan hệ suy luận quyết định đáp án đúng**: blank khớp **căn cứ/lý do** đã nêu (…때문이다, 그 이유는); **nhân-quả** (따라서/그래서/그러므로 → blank là kết quả); **đối-lập** (그러나/하지만/반면 → blank tương phản); **khái-quát** (즉/이처럼/결국 → blank tổng kết); **mục-đích** (-기 위해/도움/역할); **điều-kiện** (-으면 → hệ quả); **ví-dụ** (예를 들어/그중); **so-sánh** (처럼/와 같이/-와 달리). Map → category enum (`căn-cứ|nhân-quả|đối-lập|khái-quát|mục-đích|điều-kiện|ví-dụ|so-sánh`).
- Quét `TOPIK/48_Blanks/` theo `blank_id`:
  - `EXISTS`: append `history` `{exam, q, role: correct, reason: "{đáp án đúng/tín hiệu} → blank {nội dung}"}` + cập nhật `cue_markers`/`members` nếu có biểu hiện mới.
  - Quan hệ mới xuất hiện **≥2×** (tính cả lịch sử) mà chưa có note → đề xuất tạo từ `Blank_Template.md` (điền `blank_name`, `category`, `cue_markers`, `role_in_para`, `kill_signal`, `select_signal`, marker sync). Quan hệ 1× ghi inline (cột "Căn cứ / Phân tích", KHÔNG link) + liệt kê candidate (theo dõi ở `Blank_Stats.md`).
  - `role: distractor` (tùy chọn): khi một phương án SAI ngộ nhận/vi phạm một quan hệ rõ rệt (vd chọn mệnh đề nhân-quả trong khi mạch bài là đối-lập) → có thể log `{..., role: distractor, reason: "② mệnh đề nhân-quả — mạch bài là đối-lập"}` để quan hệ tích lũy cả 2 vai.
- **exam block** (bảng **5 cột** `# | Phương án | Nghĩa | Vai | Căn cứ / Phân tích`): đáp án đúng Vai ✅ (KHÔNG cite `[RA:]`); cột cuối phân tích vì sao đúng khớp quan hệ / vì sao phương án sai, quan hệ có note → link `[[BLK_*]]`. Thêm dòng `**Quan hệ:** {quan hệ đáp án đúng} → [[BLK_{category}]]` **mức câu** (dưới khối `**Dịch:**`, trước bảng — KHÔNG thêm cột; quan hệ 1× chưa có note → mô tả, KHÔNG link).

### 2g. Câu dạng `điền-phó-từ` (câu 19 — single-layer adverb-note, Q21)

Dạng điền phó từ chỗ trống sinh **một layer duy nhất `adverb-note`** (`TOPIK/49_Adverbs/`, tổ chức **theo chức năng phó từ**, 1 file = 1 chức năng; taxonomy trong `frontmatter_schema.md` §adverb-note). KHÔNG đụng GR/PAIR/GROUP/DIS/TOPIC/TRAP/SEQ/BLK (VC vẫn tạo ở 2d khi có cụm lexical-hóa; link `[[GR_*]]` nền khi note tồn tại).

- Với mỗi câu, xác định **chức năng phó từ quyết định đáp án đúng** (đọc quan hệ hai câu quanh chỗ trống): **bổ-sung** (또한/게다가/그리고 → câu sau thêm ý cùng chiều); **đối-lập** (오히려/반면에/그러나 → câu sau đảo hướng); **nhân-quả** (그래서/따라서/그러므로 → câu sau là kết quả); **điều-kiện** (만약/만일 → vế sau có -면/-다면/-라면); **khái-quát** (즉/이처럼/결국 → tổng kết); **suy-đoán** (아마/어쩌면); **nhấn-mạnh** (특히/역시/과연/반드시 → chỉ rõ trọng tâm); **nhượng-bộ** (물론/비록/그래도 → 물론 A…그런데 B); **lựa-chọn** (또는/혹은); **trình-tự-thời-gian** (드디어/마침내/우선). Map → category enum.
- Quét `TOPIK/49_Adverbs/` theo `adv_id`:
  - `EXISTS`: append `history` `{exam, q, role: correct, reason: "{phó từ} — {tín hiệu}"}` + cập nhật `members` nếu có phó từ mới.
  - Chức năng mới xuất hiện **≥2×** (tính cả lịch sử) mà chưa có note → đề xuất tạo từ `Adverb_Template.md` (điền `adv_name`, `category`, `members`, `kill_signal`, `select_signal`, marker sync). Chức năng 1× ghi inline (cột "Căn cứ / Phân tích", KHÔNG link) + liệt kê candidate (theo dõi ở `Adverb_Stats.md`).
  - `role: distractor` (tùy chọn): khi một phó từ SAI ngộ nhận/vi phạm một chức năng rõ rệt (vd chọn phó từ bổ-sung trong khi hai câu là đối-lập) → có thể log `{..., role: distractor, reason: "④ 반드시 — câu không mang nghĩa 'nhất định'"}` để chức năng tích lũy cả 2 vai.
- **exam block** (bảng **5 cột** `# | Phương án | Nghĩa | Vai | Căn cứ / Phân tích`): đáp án đúng Vai ✅ (KHÔNG cite `[RA:]`); cột cuối phân tích vì sao phó từ đúng khớp mạch / vì sao phó từ sai, chức năng có note → link `[[ADV_*]]`. Thêm dòng `**Chức năng:** {chức năng đáp án đúng} → [[ADV_{category}]]` **mức câu** (dưới khối `**Dịch:**`, trước bảng — KHÔNG thêm cột; chức năng 1× chưa có note → mô tả, KHÔNG link).
- **Câu 19 & 20 chung đoạn văn (Q2):** block Câu 19 giữ `### Chú giải chi tiết` **đầy đủ**; block Câu 20 (dạng `câu-giống-đoạn-văn-dài` — xem 2c, sinh dual-layer trap ≥2× + topic 화제 1×) render blockquote 문제/Dịch + `**Chủ đề:**` + bảng phương án, phần chú giải chỉ **một dòng trỏ** `> **Chú giải chi tiết:** đoạn văn dùng chung — xem **Câu 19** phía trên.`
...
- Sequence note (kỹ thuật nối câu) mới (chỉ khi ≥2×): điền `cue_id`(=tên file), `cue_name`, `category`, `role_in_order`, `members` (khi có), `kill_signal` + `select_signal` (bắt buộc), `history` gộp, marker sync; `correct_count`/`distractor_count` để trống.
- **Blank note (quan hệ suy luận chỗ trống) mới (chỉ khi ≥2×, Q20):** điền `blank_id`(=tên file), `blank_name`, `category` (căn-cứ/nhân-quả/đối-lập/khái-quát/mục-đích/điều-kiện/ví-dụ/so-sánh), `cue_markers`/`role_in_para` (khi có), `kill_signal` + `select_signal` (bắt buộc), `history` gộp, marker sync; `correct_count`/`distractor_count` để trống. Câu 16-18 sinh **chỉ blank-note** (KHÔNG GR/PAIR/GROUP/DIS/TOPIC/TRAP/SEQ); exam block có dòng `**Quan hệ:** … → [[BLK_*]]` mức câu. Quan hệ 1× → candidate ở `Blank_Stats.md` + mô tả inline (KHÔNG link).
