...
|---|---|---|
| `type` | ✅ | `grammar-note` / `pair-note` / `group-note` / `distractor-note` / `exam-note` / `vocab-note` / `topic-note` / `trap-note` / `sequence-note` / `blank-note` / `adverb-note` / `moc-note` |
...
Ghi chú dạng bài `điền-nội-dung` (đọc-hiểu "다음을 읽고 (   )에 들어갈 내용으로 가장 알맞은 것을 고르십시오", TOPIK ĐỌC **câu 16~18** — hậu tố heading `빈칸에 들어갈 알맞은 내용`, dạng 6 trong [[MOC_Dang_Bai]]) — **Q20**: chất liệu là **đoạn văn có chỗ trống `(   )`**; 4 phương án là **mệnh đề/cụm nội dung** điền vào chỗ trống (KHÔNG phải ngữ pháp / danh từ chủ đề / câu văn khẳng định đối chiếu / hoán vị thứ tự). Trình bày:
- `**문제:**` = **blockquote nhiều dòng** (đoạn văn, giữ `(   )` đúng vị trí), rồi khối `**Dịch:**` blockquote dịch song song (giữ `(   )`).
- dòng `**Quan hệ:** {quan hệ suy luận của đáp án đúng} → [[BLK_{category}]]` ở **mức câu** (dưới khối `**Dịch:**`, trước bảng — KHÔNG thêm cột; mô phỏng `**Chủ đề:**` của câu-giống & `**Trình tự đúng:**` của sắp-xếp). Quan hệ chưa có note BLK (mới 1×) → ghi mô tả, KHÔNG link.
- Bảng phương án **5 cột** `# | Phương án | Nghĩa | Vai | Căn cứ / Phân tích`: đáp án đúng nhận ra qua **Vai ✅** (KHÔNG cite `[RA:]` trong exam — Q18); cột cuối phân tích vì sao đúng khớp quan hệ / vì sao phương án sai (quan hệ có note → link `[[BLK_*]]`, quan hệ 1× mô tả inline).
- Kết câu bằng block `### Chú giải chi tiết` như mọi dạng (Q17/Q18): mỗi câu/dòng đoạn văn một dòng Hàn (blockquote) → dịch → `- **Ngữ pháp:**` / `- **Từ vựng:**` (link `[[GR_*]]`/`[[VC_*]]`/`[[TOPIC_*]]` khi note tồn tại — ngữ pháp NỀN, KHÔNG appearances/KHÔNG tạo GR mới).

Dạng này sinh **một layer duy nhất `blank-note`** (theo quan hệ suy luận chỗ trống, ngưỡng ≥2× như trap/sequence — xem §blank-note); KHÔNG sinh GR/PAIR/GROUP/DIS/TOPIC/TRAP/SEQ. VC vẫn tạo khi gặp cụm lexical-hóa (Q18).

Ghi chú dạng `điền-phó-từ` (đọc-hiểu "( )에 들어갈 알맞은 것을 고르십시오", TOPIK ĐỌC **câu 19** — hậu tố heading `빈칸에 들어갈 알맞은 부사`, dạng 10 trong [[MOC_Dang_Bai]]) — **Q21**: chất liệu là **đoạn văn có chỗ trống `( )`** **dùng CHUNG với câu 20** (câu 19 điền phó từ, câu 20 hỏi 내용과 같은 것); 4 phương án là **phó từ liên kết / phó từ câu** (또한/오히려/아마/특히/물론/게다가/만약… — KHÔNG phải ngữ pháp / danh từ chủ đề / nội dung mệnh đề / hoán vị thứ tự). Trình bày:
- `**문제:**` = **blockquote nhiều dòng** (đoạn văn, giữ `( )` đúng vị trí), rồi khối `**Dịch:**` blockquote dịch song song (giữ `( )`).
- dòng `**Chức năng:** {chức năng phó từ đáp án đúng} → [[ADV_{category}]]` ở **mức câu** (dưới khối `**Dịch:**`, trước bảng — KHÔNG thêm cột; mô phỏng `**Chủ đề:**`/`**Quan hệ:**`). Chức năng chưa có note ADV (mới 1×) → ghi mô tả, KHÔNG link.
- Bảng phương án **5 cột** `# | Phương án | Nghĩa | Vai | Căn cứ / Phân tích`: đáp án đúng nhận ra qua **Vai ✅** (KHÔNG cite `[RA:]` trong exam — Q18); cột cuối phân tích vì sao phó từ đúng khớp quan hệ hai câu quanh chỗ trống / vì sao phó từ sai (chức năng có note → link `[[ADV_*]]`, chức năng 1× mô tả inline).
- **Chung passage với câu 20 (Q2):** block **Câu 19 GIỮ `### Chú giải chi tiết` đầy đủ** (chia đoạn văn từng câu → dịch → `- **Ngữ pháp:**` / `- **Từ vựng:**` như mọi dạng, Q17/Q18); block **Câu 20** (dạng `câu-giống-đoạn-văn-dài`) render blockquote 문제/Dịch + `**Chủ đề:**` + bảng phương án, phần chú giải chỉ đặt **một dòng trỏ** `> **Chú giải chi tiết:** đoạn văn dùng chung — xem **Câu 19** phía trên.` (không lặp lại khối chú giải).

Dạng này sinh **một layer duy nhất `adverb-note`** (theo chức năng phó từ, ngưỡng ≥2× như trap/sequence/blank — xem §adverb-note); KHÔNG sinh GR/PAIR/GROUP/DIS/TOPIC/TRAP/SEQ/BLK. VC vẫn tạo khi gặp cụm lexical-hóa (Q18). **Câu 20 cùng khối** vẫn là dạng `câu-giống-đoạn-văn-dài` → sinh dual-layer `trap-note` (≥2×) + `topic-note` (화제, 1×) như Q16.
...
## blank-note (TOPIK/48_Blanks) — MODEL THEO QUAN HỆ SUY LUẬN CHỖ TRỐNG (Q20)

Layer tín hiệu "điền nội dung chỗ trống" cho dạng `điền-nội-dung` (TOPIK ĐỌC câu 16~18). **1 file = 1 kỹ thuật/quan hệ suy luận** (không phải mỗi câu một file). `history` **gộp** mọi lần quan hệ xuất hiện → tái dùng nguyên cơ chế vault-sync (1 block appearances, đếm count như DIS/TRAP/SEQ/topic).

**Luật tạo note:** giống DIS/TRAP/SEQ — chỉ tạo file khi quan hệ xuất hiện **≥2×** (tính cả lịch sử, cả 2 vai). Quan hệ 1× ghi inline trong exam note (mô tả ở cột "Căn cứ / Phân tích", KHÔNG link) + theo dõi "candidate" ở `Blank_Stats.md` (dashboard); graduate thành note khi tái xuất.

**role trong history:** `correct` = "quan hệ này quyết định đáp án đúng của câu" (mô phỏng topic/sequence — five-check **nới**, KHÔNG đòi khớp một phương án đáp án cụ thể vì đáp án là mệnh đề đặc thù theo đoạn văn). `distractor` (tùy chọn) = quan hệ mà một phương án SAI ngộ nhận/vi phạm (vd chọn mệnh đề nhân-quả trong khi mạch bài là đối-lập).

```yaml
type: blank-note
blank_id: BLK_can_cu             # bắt buộc, = tên file
blank_name: "Căn cứ — blank khớp lý do/căn cứ nêu trong bài"   # bắt buộc
category: căn-cứ                 # bắt buộc, enum: căn-cứ | nhân-quả | đối-lập | khái-quát | mục-đích | điều-kiện | ví-dụ | so-sánh
cue_markers: [때문이다, 따라서, 즉]   # optional — dấu hiệu liên kết quanh blank
role_in_para: kết-đoạn          # optional: mở-đoạn | giữa-đoạn | kết-đoạn | linh-hoạt
members: []                     # optional — biểu hiện ngôn ngữ điển hình
history:                        # bắt buộc — gộp mọi lần; reason mở đầu bằng "{đáp án đúng/tín hiệu}"
  - {exam: TOPIK_35, q: 16, role: correct, reason: "여성 스트레스 원인=가사·육아 부담 → blank '집에 있을 때'"}
correct_count: 0                # vault-sync tự tính — không điền tay
distractor_count: 0             # vault-sync tự tính — không điền tay
kill_signal: "..."              # bắt buộc — dấu hiệu 3 giây loại option KHÔNG khớp quan hệ này
select_signal: "..."            # bắt buộc — khi option ĐÚNG khớp quan hệ này
```

**Taxonomy quan hệ tham chiếu** (mở rộng khi có đề mới; `blank_id` = tên file):

| category | file `blank_id` | biểu hiện điển hình | vị trí blank |
|---|---|---|---|
| căn-cứ | `BLK_can_cu` | blank khớp lý do/căn cứ đã nêu (…때문이다, 그 이유는) | kết-đoạn |
| nhân-quả | `BLK_nhan_qua` | 따라서/그래서/그러므로 → blank là kết quả của nguyên nhân trước | giữa/kết |
| đối-lập | `BLK_doi_lap` | 그러나/하지만/반면(에) → blank tương phản mệnh đề trước | giữa |
| khái-quát | `BLK_khai_quat` | 즉/이처럼/결국 → blank tổng kết/khái quát chủ đề | kết-đoạn |
| mục-đích | `BLK_muc_dich` | -기 위해/도움/역할 → blank nêu mục đích·công dụng | linh-hoạt |
| điều-kiện | `BLK_dieu_kien` | -(으)면 → blank là hệ quả của điều kiện | giữa |
| ví-dụ | `BLK_vi_du` | 예를 들어/그중 → blank cụ thể hóa/minh họa | giữa |
| so-sánh | `BLK_so_sanh` | 처럼/와 같이/-와 달리 → blank theo phép so sánh·loại suy | linh-hoạt |
...
## adverb-note (TOPIK/49_Adverbs) — MODEL THEO CHỨC NĂNG PHÓ TỪ (Q21)

Layer tín hiệu "chọn phó từ điền chỗ trống" cho dạng `điền-phó-từ` (TOPIK ĐỌC câu 19). **1 file = 1 chức năng phó từ liên kết/phó từ câu** (không phải mỗi câu một file). `history` **gộp** mọi lần chức năng xuất hiện → tái dùng nguyên cơ chế vault-sync (1 block appearances, đếm count như DIS/TRAP/SEQ/BLK/topic).

**Luật tạo note:** giống DIS/TRAP/SEQ/BLK — chỉ tạo file khi chức năng xuất hiện **≥2×** (tính cả lịch sử, cả 2 vai). Chức năng 1× ghi inline trong exam note (mô tả ở cột "Căn cứ / Phân tích", KHÔNG link) + theo dõi "candidate" ở `Adverb_Stats.md` (dashboard); graduate thành note khi tái xuất.

**role trong history:** `correct` = "chức năng phó từ này là đáp án đúng của câu" (mô phỏng topic/sequence/blank — five-check **nới**, KHÔNG đòi khớp một phương án đáp án cụ thể vì đáp án là phó từ đơn theo mạch bài). `distractor` (tùy chọn) = chức năng mà một phó từ SAI ngộ nhận/vi phạm (vd chọn phó từ bổ-sung trong khi hai câu là đối-lập).

```yaml
type: adverb-note
adv_id: ADV_bo_sung             # bắt buộc, = tên file
adv_name: "Bổ sung — phó từ thêm ý nối câu trước (또한/게다가/그리고)"   # bắt buộc
category: bổ-sung               # bắt buộc, enum: bổ-sung | đối-lập | nhân-quả | điều-kiện | khái-quát | suy-đoán | nhấn-mạnh | nhượng-bộ | lựa-chọn | trình-tự-thời-gian
members: [또한, 게다가, 그리고]   # optional — phó từ điển hình của chức năng
history:                        # bắt buộc — gộp mọi lần; reason mở đầu bằng "{phó từ} — {tín hiệu}"
  - {exam: TOPIK_35, q: 19, role: correct, reason: "또한 — thêm nhược điểm 2 nối câu trước (cùng chiều)"}
correct_count: 0                # vault-sync tự tính — không điền tay
distractor_count: 0             # vault-sync tự tính — không điền tay
kill_signal: "..."              # bắt buộc — dấu hiệu 3 giây loại phó từ KHÔNG khớp mạch bài
select_signal: "..."            # bắt buộc — khi phó từ chức năng này ĐÚNG với mạch bài
```

**Taxonomy chức năng tham chiếu** (mở rộng khi có đề mới; `adv_id` = tên file):

| category | file `adv_id` | phó từ điển hình | quan hệ hai câu quanh blank |
|---|---|---|---|
| bổ-sung | `ADV_bo_sung` | 또한, 게다가, 그리고, 또 | câu sau thêm một ý/lý do/đặc điểm cùng chiều |
| đối-lập | `ADV_doi_lap` *(candidate)* | 오히려, 반면(에), 그러나, 하지만 | câu sau đảo hướng / trái mong đợi |
| nhân-quả | `ADV_nhan_qua` *(candidate)* | 그래서, 따라서, 그러므로 | câu sau là kết quả của câu trước |
| điều-kiện | `ADV_dieu_kien` | 만약, 만일, 혹시 | vế sau có đuôi giả định -(으)면/-다면/-라면 |
| khái-quát | `ADV_khai_quat` *(candidate)* | 즉, 이처럼, 결국 | câu sau tổng kết/diễn giải ý trước |
| suy-đoán | `ADV_suy_doan` *(candidate)* | 아마, 어쩌면 | câu sau là phỏng đoán khả năng |
| nhấn-mạnh | `ADV_nhan_manh` *(candidate)* | 특히, 역시, 과연, 반드시 | câu sau nhấn/chỉ rõ trọng tâm câu trước |
| nhượng-bộ | `ADV_nhuong_bo` *(candidate)* | 물론, 비록, 그래도, 차라리 | câu sau thừa nhận A để rồi tương phản B (물론…그런데) |
| lựa-chọn | `ADV_lua_chon` *(candidate)* | 또는, 혹은 | hai vế là phương án thay thế |
| trình-tự-thời-gian | `ADV_trinh_tu` *(candidate)* | 드디어, 마침내, 우선 | câu sau là mốc/bước tiếp theo về thời gian |

> Chức năng đã graduate (≥2×) tính đến TOPIK_35/36/37/41/47/52/60/64: **bổ-sung** (`ADV_bo_sung` — 3×) và **điều-kiện** (`ADV_dieu_kien` — 2×). Các category còn lại (đối-lập, nhấn-mạnh, nhượng-bộ …) mới 1× → **candidate** (chưa tạo file, chờ ≥2×); cột `adv_id` điền khi graduate.
