...
| Q20 | Domain topik kích hoạt dạng đọc-hiểu **`điền-nội-dung`** (TOPIK ĐỌC **câu 16~18**, đề bài "다음을 읽고 (   )에 들어갈 내용으로 가장 알맞은 것을 고르십시오", hậu tố heading `빈칸에 들어갈 알맞은 내용`, **dạng 6** trong `MOC_Dang_Bai` — đổi `dự phòng` → `đang dùng`): chất liệu là **đoạn văn có chỗ trống ( )**, 4 phương án là **mệnh đề/cụm nội dung** (KHÔNG phải ngữ pháp / danh từ chủ đề / câu khẳng định đối chiếu / hoán vị). Sinh **một layer mới `blank-note`** (`TOPIK/48_Blanks/`, prefix `BLK_`) tổ chức **theo quan hệ suy luận chỗ trống** (`căn-cứ\|nhân-quả\|đối-lập\|khái-quát\|mục-đích\|điều-kiện\|ví-dụ\|so-sánh`), ngưỡng **≥2×** như trap/sequence (quan hệ 1× → inline + candidate ở `Blank_Stats.md`), `role: correct` = "quan hệ quyết định đáp án đúng của câu" (five-check nới như topic/sequence); **KHÔNG** sinh GR/PAIR/GROUP/DIS/TOPIC/TRAP/SEQ (VC vẫn tạo khi có cụm lexical-hóa). Exam block **5 cột** `# \| Phương án \| Nghĩa \| Vai \| Căn cứ / Phân tích` + dòng `**Quan hệ:** {quan hệ đáp án đúng} → [[BLK_{category}]]` mức câu (dưới Dịch, trước bảng — KHÔNG thêm cột); render 문제/Dịch blockquote giữ `( )` + `### Chú giải chi tiết` như mọi dạng (Q17/Q18). vault-sync đếm count + render marker + sinh dashboard `Blank_Stats.md`. Chi tiết `frontmatter_schema.md` §blank-note. Backfill 8 đề (35/36/37/41/47/52/60/64) câu 16-18. |
| Q21 | Domain topik kích hoạt dạng đọc-hiểu **`điền-phó-từ`** (TOPIK ĐỌC **câu 19**, đề bài "( )에 들어갈 알맞은 것을 고르십시오", hậu tố heading `빈칸에 들어갈 알맞은 부사`, **dạng 10** trong `MOC_Dang_Bai`): chất liệu là **đoạn văn có chỗ trống ( ) dùng CHUNG với câu 20**; 4 phương án là **phó từ liên kết/phó từ câu** (또한/오히려/아마/특히/물론/게다가/만약… — KHÔNG phải ngữ pháp / danh từ chủ đề / mệnh đề nội dung / hoán vị). Sinh **một layer mới `adverb-note`** (`TOPIK/49_Adverbs/`, prefix `ADV_`) tổ chức **theo chức năng phó từ** (`bổ-sung\|đối-lập\|nhân-quả\|điều-kiện\|khái-quát\|suy-đoán\|nhấn-mạnh\|nhượng-bộ\|lựa-chọn\|trình-tự-thời-gian`), ngưỡng **≥2×** như trap/sequence/blank (chức năng 1× → inline + candidate ở `Adverb_Stats.md`), `role: correct` = "chức năng quyết định đáp án đúng của câu" (five-check nới như topic/sequence/blank); **KHÔNG** sinh GR/PAIR/GROUP/DIS/TOPIC/TRAP/SEQ/BLK (VC vẫn tạo khi có cụm lexical-hóa). Exam block **5 cột** `# \| Phương án \| Nghĩa \| Vai \| Căn cứ / Phân tích` + dòng `**Chức năng:** {chức năng đáp án đúng} → [[ADV_{category}]]` mức câu (dưới Dịch, trước bảng — KHÔNG thêm cột). **Câu 19 & 20 chung đoạn văn (Q2):** block Câu 19 giữ `### Chú giải chi tiết` đầy đủ, block Câu 20 (dạng `câu-giống-đoạn-văn-dài`) render blockquote 문제/Dịch + `**Chủ đề:**` + bảng phương án nhưng phần chú giải chỉ một dòng trỏ "xem Câu 19"; câu 20 vẫn sinh dual-layer trap (≥2×) + topic (화제, 1×) như Q16. vault-sync đếm count + render marker + sinh dashboard `Adverb_Stats.md`. Chi tiết `frontmatter_schema.md` §adverb-note. Backfill 8 đề (35/36/37/41/47/52/60/64) câu 19-20. |
